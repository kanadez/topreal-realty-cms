function PropertyComparison(){
    ...
    this.refresh = function(){ // добавляет к старому списку новую недвижимость если есть
        ...
        $.post("/api/client/getlistbyproperty.json",{
            property_id: this.subject.data.id,
            mode: "new",
            ...
            if (adv_items.length > 0){
                ...
                if ($('#comparison_timestamp_offset').val() != 0){
                    propcomp.subject.clients = adv_items;
                    propcomp.adv_items = [];
                }
                else{
                    for (var i = 0; i < adv_items.length; i++){
                        ...
                        for (var z = 0; z < propcomp.subject.clients.length; z++){
                            if (adv_items[i].id == propcomp.subject.clients[z].id){
                                exist = true;
                            }
                        }

                        if (!exist){
                            propcomp.adv_items.push(adv_items[i]);
                        }
                    }
                }
                ...
                for (var i = 0; i < propcomp.adv_items.length; i++){
                    $('#comparison_table').append(propcomp.generateListRow(propcomp.adv_items[i]));
                }
    ...
    this.generateList = function(){
        $('#comparison_finded_span').text(this.subject.clients.length);
        
        for (var i = 0; i < this.subject.clients.length; i++){
            $('#comparison_table').append(this.generateListRow(this.subject.clients[i]));
        ...
        $('.comparison_check')
                ...
                .on("ifUnchecked", function(event){
                    ...
                    if ($('.comparison_check:checked').length == 0 && $('.hl_green').length === 0){
    ...
    this.generateListRow = function(item){
        ...
        var data = {};
        data.price = utils.numberWithCommas(item.price_from)+' - '+utils.numberWithCommas(item.price_to)+' '+this.subject.form_options.currency[item.currency_id]["symbol"];
        ...
        $('#comparison_table').show()
                .append("<tr client='"+item.id+"' ondblclick='propcomp.openCard("+item.id+")' id='row_"+item.id+"'>\n\
                            <td class='comparison_check_td'><input class='icheck comparison_check' type='checkbox' value='"+item.id+"' name='' /></td>\n\
                            <td class='comparison_types_td'>"+(item.name != null ? item.name : "")+"</td>\n\
                            <td class='comparison_id_td'>"+item.id+"</td>\n\
                            <td class='comparison_price_td'>"+data.price+"</td>\n\
                            <td style='text-align:center;' class='comparison_history_open_td'><button onclick='propcomp.openHistory("+item.id+")' style='display:none;' title='xsl_locale_open_item_history__' locale_title='open_item_history' type='button' class='btn btn-primary btn_narrow comparison_history_button'><i class='fa fa-folder-open'></i></button></td>\n\
                            <td class='comparison_condition_td'><select onchange='propcomp.addChange(\"condition\", this)' disabled>"+condition_select_options+"</select></td>\n\
                            ...
                            <td class='comparison_buttons_td'>\n\
                                <button onclick='propcomp.editRow("+item.id+")' title='xsl_locale_comparison_item_edit__' locale_title='comparison_item_edit' type='button' class='btn btn-primary btn_narrow comparison_edit_button'><i class='fa fa-pencil'></i></button>\n\
                                <button onclick='propcomp.removeRow("+item.id+")' title='' locale_title='' style='display:none;' type='button' class='btn btn-danger comparison_delete_button'><i class='fa fa-times'></i></button>\n\
                                <button onclick='propcomp.undoRow()' style='display:none;' type='button' title='xsl_locale_comparison_item_undo__' locale_title='comparison_item_undo' class='btn btn-default btn_narrow comparison_undo_button'><i class='fa fa-undo'></i></button>\n\
                                <button onclick='propcomp.restoreRow("+item.id+")' title='' locale_title='' style='display:none;' type='button' class='btn btn-default comparison_restore_button'><i class='fa fa-refresh'></i></button>\n\
                            </td>\n\
                        <tr>");
    };
    ...
    this.openCard = function(id){
        if (this.editing_row === null){
            location.href="client?id="+id+"&property="+property.data.id;
        }
        else{
    ...
    this.editRow = function(id){
        ...
        $('.comparison_editing_tr .comparison_remarks_td').html("<input placeholder='"+localization.getVariable("remark")+"' onkeyup='propcomp.addChange(\"remarks\", this)' value='"+remarks_text+"'/>");
        $('.comparison_editing_tr .comparison_edit_button').attr({
            onclick: "propcomp.saveRow()",
    ...
    this.saveRow = function(){
        if (
            ...
            this.event.action_old == undefined && 
        ...
        $.post("/api/propertycomp/addevent.json",{
            property: this.subject.data.id,
            client: $('.comparison_editing_tr').attr("client"),
            ...
            if (response.error != undefined){
                ...
                $('.comparison_editing_tr .comparison_edit_button').attr({
                    onclick: "propcomp.editRow("+response+")",
    ...
    this.undoRow = function(){
        ...
        $('.comparison_editing_tr .comparison_edit_button').attr("onclick", "propcomp.editRow("+$('.comparison_editing_tr').attr("client")+")");
        $('.comparison_editing_tr .comparison_edit_button > i').removeClass("fa-floppy-o").addClass("fa-pencil");
        $('.comparison_editing_tr .comparison_undo_button').hide();
    ...
    /*this.openRemoveRowDialog = function(id){
        ...
        var buttons_block = "<div class='comparison_remove_buttons_block'>\n\
                                ...
                                <button onclick='propcomp.removeRow()' type='button' class='btn btn-primary comparison_delete_button'>"+localization.getVariable("yes")+"</button>\n\
                                <button onclick='propcomp.removeRow()' type='button' class='btn btn-default comparison_delete_button'>"+localization.getVariable("no")+"</button>\n\
                            </div>";
    ...
    this.deleteChecked = function(){
        ...
        try{
            ...
            for (var i = 0; i < propcomp.checked_values.length; i++){
                $('#comparison_table #row_'+propcomp.checked_values[i]).remove();
            }
            
            $('#comparison_finded_span').text($('.comparison_check').length);
            $('#comparison_marked_span').text($('.comparison_check:checked').length);*/

            //propcomp.checked_values = null;
            
            $.post("/api/propertycomp/massdelete.json",{
                property: this.subject.data.id,
                clients: JSON.stringify(this.checked_values)
            },function (response){
                if (response.error != undefined){
                    showErrorMessage(response.error.description);
                }
    ...
    this.massHide = function(){
        ...
        try{
            ...
            $.post("/api/propertycomp/masshide.json",{
                property: this.subject.data.id,
                clients: JSON.stringify(this.checked_values)
            }, null);
        }
    ...
    this.massUnhide = function(){
        ...
        $.post("/api/propertycomp/massunhide.json",{
            property: this.subject.data.id
        }, null);
    };
    ...
    this.apply = function(){
         $.post("/api/propertycomp/apply.json",{
            property: this.subject.data.id
        },function (response){
            propcomp.data = response;
            
            for (var i = 0; i < response.length; i++){
                for (var z = 0; z < propcomp.subject.clients.length; z++){
                    if (propcomp.subject.clients[z].id == response[i].client){
                        var client = $('#comparison_table #row_'+response[i].client);
                        
                        switch (response[i].event){
                            case "action_change":
                                client.children('.comparison_action_td')
                                        .children('select')
                                        .val(response[i].current_state)
                                        .children(':first-child').attr("disabled", "true");
                                client.children('.comparison_date_td').text(utils.convertTimestampForDatepicker(response[i].timestamp));
                                client.children('.comparison_agent_td').attr("agent_id", response[i].author);
                            break;
                            case "condition_change":
                                client.children('.comparison_condition_td')
                                        .children('select')
                                        .val(response[i].current_state)
                                        .children(':first-child').attr("disabled", "true");
                                client.children('.comparison_date_td').text(utils.convertTimestampForDatepicker(response[i].timestamp));
                                ...
                                if (response[i].current_state != 0){
                                    client.children().children('.comparison_history_button').show();
                                    client.children('.comparison_check_td').children().children('input').attr("hide_only", "true");
                            ...
                            break;
                            case "remarks_change":
                                client.children('.comparison_remarks_td').text(response[i].current_state);
                                client.children('.comparison_date_td').text(utils.convertTimestampForDatepicker(response[i].timestamp));
                                client.children('.comparison_agent_td').attr("agent_id", response[i].author);
                            break;
                            case "delete":
            ...
            $('.comparison_agent_td').each(function(){
                if ($(this).attr("agent_id") != undefined){
                    for (var i = 0; i < property.agent_list.length; i++){
                        if (property.agent_list[i].id == $(this).attr("agent_id")){
                            $(this).text(property.agent_list[i].name);
                        }
                    }
                }
            });
    ...
    this.propose = function(){
        $.post("/api/propertycomp/propose.json",{
            client: this.subject.data.id
        }, null);
    };
    ...
    this.getHided = function(){
        $.post("/api/propertycomp/gethided.json",{
            property: this.subject.data.id
        },function (response){
            if (response.length > 0){
                ...
                for (var i = 0; i < response.length; i++){
                    $('#comparison_table #row_'+response[i].client)
                        .addClass("comparison_hidden_row")
                        .hide();
                }
    ...
    this.showEventsModal = function(){
        ...
        $.post("/api/propertycomp/geteventsforproperty.json",{
            ...
            for (var i = 0; i < response.length; i++){
                $('#comparison_events_table').append(propcomp.generateEventsListRow(response[i]));
    ...
    this.generateEventsListRow = function(item){
        ...
        for (var z = 0; z < types.length; z++){
            data.types += "<span locale='"+client.form_options.property_type[types[z]]+"'></span>"+(z === types.length-1 ? "" : "/");
        }
        
        data.price = "";
        data.price += utils.numberWithCommas(item.price)+" "+this.subject.form_options.currency[item.currency_id]["symbol"];
        ...
        $('#comparison_events_table').show()
                .append("<tr property='"+item.id+"' ondblclick='location.href=\"property?id="+item.id+"&client_events="+client.data.id+"\"' id='row_"+item.id+"'>\n\
