...
function Forgot(){
    ...
    this.getCode = function(){
        ...
        utils.htmlSpinner('signin_submit_button');
        
        $.post("/api/forgot/getcode.json",{
            email: $('#email').val().trim(),
            locale: localization.locale_value != null ? localization.locale_value : "en"
        },function (response){
            utils.removeHtmlSpinner('signin_submit_button');
            
            if (response.error != undefined){
                $('#error_div').show().attr("locale", response.error.description);
                localization.toLocale();
            }
            else{
                $('#code_input_wrapper').animate({height:"+=60px", overflow:"visible"}, 200, function(){
                    $('#signin_submit_button').attr({onclick: "forgot.tryCode()"});
                }).parent().show();
            }
        });
    };
    
    this.tryCode = function(){
        if ($('#email').val().trim().length === 0){
           this.highlightField($('#email'));
        ...
        if ($('#code_input').val().trim().length === 0){
           this.highlightField($('#code_input'));
           
           return false;
        }
        
        $.post("/api/forgot/trycode.json",{
            email: $('#email').val().trim(),
            ...
            if (response.error != undefined){
                ...
                $('#passwords_input_wrapper').show().animate({height:"+=120px", overflow:"visible"}, 200, function(){
                    $('#signin_submit_button').attr({onclick: "forgot.setPassword()", locale: "save_password"}).text(localization.getVariable("save_password"));
                }).parent().show();
            }
        });
    };
    ...
    this.setPassword = function(){
        var password = MD5($('#new_password_input').val().trim());
        var password_again = MD5($('#new_password_again_input').val().trim());
        ...
        if ($('#new_password_input').val().trim().length === 0){
           this.highlightField($('#new_password_input'));
           
           return false;
        }
        
        if ($('#new_password_again_input').val().trim().length === 0){
           this.highlightField($('#new_password_again_input'));
           
           return false;
        }
        
        if (password !== password_again){
            $('#password_error_div').show().text(localization.getVariable("password_match_alert"));
            
            return false;
        }
        
        $.post("/api/forgot/resetpass.json",{
            email: $('#email').val().trim(),
            code: $('#code_input').val().trim(),
            password: password
        },function (response){
            if (response.error != undefined){
                ...
                $('.to_hide').hide();
                $('#login_input_wrapper').parent().show();
                $('#signin_submit_button').attr("onclick", 'location.href="login";').text(localization.getVariable("sign_in_a"));
            }
        });
    };
